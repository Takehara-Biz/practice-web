...
export class PetApiController {
  ...
  findPetsByStatus(@Query('status', new DefaultValuePipe('available')) status: 'available' | 'pending' | 'sold' | undefined, @Req() request: Request): Array<Pet> | Promise<Array<Pet>> | Observable<Array<Pet>> {
    return this.petApi.findPetsByStatus(status, request);
  }

  @Get('/pet/findByTags')
  findPetsByTags(@Query('tags') tags: Array<string> | undefined, @Req() request: Request): Array<Pet> | Promise<Array<Pet>> | Observable<Array<Pet>> {
  ...
  @Post('/pet/:petId')
  updatePetWithForm(@Param('petId') petId: number, @Query('name') name: string | undefined, @Query('status') status: string | undefined, @Req() request: Request): void | Promise<void> | Observable<void> {
    return this.petApi.updatePetWithForm(petId, name, status, request);
  }

  @Post('/pet/:petId/uploadImage')
  uploadFile(@Param('petId') petId: number, @Query('additionalMetadata') additionalMetadata: string | undefined, @Body() body: Blob | undefined, @Req() request: Request): ApiResponse | Promise<ApiResponse> | Observable<ApiResponse> {
    return this.petApi.uploadFile(petId, additionalMetadata, body, request);
  }
