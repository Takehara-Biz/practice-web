...
| [**getPetById**](PetApi.md#getpetbyid) | **GET** /pet/{petId} | Find pet by identifier. |
...
async function example() {
  ...
  const body = {
    // 'available' | 'pending' | 'sold' | Status values that need to be considered for filter (optional)
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **status** | `available`, `pending`, `sold` | Status values that need to be considered for filter | [Optional] [Defaults to `&#39;available&#39;`] [Enum: available, pending, sold] |
...
async function example() {
  ...
  const body = {
    // Array<string> | Tags to filter by (optional)
    tags: ...,
